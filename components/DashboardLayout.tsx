...
import { Role, SiteContent, User } from '../types';
...
const navItems = {
  ...
  [Role.WAREHOUSE_MANAGER]: [
    { id: 'dashboard', label: 'Dashboard', icon: HomeIcon },
    { id: 'orders', label: 'Warehouse Orders', icon: PackageIcon },
...
const RoleBadge: React.FC<{ role: Role }> = ({ role }) => {
    const roleStyles: { [key in Role]: string } = {
        [Role.CUSTOMER]: 'bg-blue-500 text-blue-100',
        [Role.ORDER_PROCESSOR]: 'bg-green-500 text-green-100',
        [Role.WAREHOUSE_MANAGER]: 'bg-orange-500 text-orange-100',
        [Role.ADMIN]: 'bg-purple-500 text-purple-100',
...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ userRole, activeView, setActiveView, onLogout, children, user }) => {
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-gray-800/30 p-4 shadow-md flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            {user && <LiveNotifications user={user} onNotificationClick={(notification) => {
