...
import { User, Message, Order, Role } from '../types';
import { getMessagesByOrderId, sendMessage, getUserOrders, getAllOrders } from '../services/api';
...
const MessagesPage: React.FC<MessagesPageProps> = ({ user, initialConversationId }) => {
  ...
  useEffect(() => {
    // Fetch conversations (orders the user is part of)
    const fetchConversations = async () => {
      try {
        let userConversations: Order[] = [];

        // Customers see their own orders, staff see all orders
        if (user.role === Role.CUSTOMER) {
          userConversations = await getUserOrders(user.id);
        } else {
          // Staff roles (ORDER_PROCESSOR, WAREHOUSE_MANAGER, ADMIN, SUPER_ADMIN)
          userConversations = await getAllOrders();
        }
