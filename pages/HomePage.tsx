...
const HomePage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen text-white font-sans">
      ...
      {content && content.advertisements.length > 0 && (
        <section className="py-10">
          <div className="container mx-auto px-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {content.advertisements.filter(ad => ad.isActive).map(ad => (
                <a
                  key={ad.id}
                  href={ad.linkUrl || '#'}
                  target={ad.linkUrl ? '_blank' : '_self'}
                  rel="noopener noreferrer"
                  className="block bg-white/10 backdrop-blur-sm rounded-lg overflow-hidden hover:bg-white/20 transition-all duration-300 transform hover:scale-105"
                >
