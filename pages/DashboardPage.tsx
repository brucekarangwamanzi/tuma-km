...
import { Role, User, Order, OrderStatus, VerificationRequest, SiteContent, Company, HeroMedia, Advertisement } from '../types';
...
import { submitVerificationRequest, getPendingVerificationRequests, approveVerificationRequest, rejectVerificationRequest, getUsers, createOrderRequest, getSiteContent, updateSiteContent, updateOrderStatus, updateUserRole, getAdvertisements, createAdvertisement, updateAdvertisement, deleteAdvertisement, createHeroMedia, deleteHeroMedia } from '../services/api';
    ...
    return (
        <div className="space-y-6">
            <div className="bg-white/10 p-6 rounded-lg">
                ...
                <p className="text-gray-300">Here's a quick overview of your account and live updates.</p>
            </div>
            
            {/* Live Activity Feed for Customers */}
            {user.role === Role.CUSTOMER && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                        <h3 className="text-lg font-semibold text-white">Recent Order Status</h3>
                        <LiveOrderStatus 
                            orderId="order1" 
...
const UserOrders: React.FC<{ onViewDetails: (orderId: string) => void, user: User }> = ({ onViewDetails, user }) => {
    ...
    useEffect(() => {
        const fetchUserOrders = async () => {
            try {
                // Assuming there's an API to get user orders, for now we'll use a placeholder
                // const orders = await getUserOrders(user.id);
                setUserOrders([]); // Placeholder
            } catch (error) {
    ...
    const processedOrders = useMemo(() => {
        let orders = userOrders.filter(order => {
...
const AllOrders: React.FC<{ onViewDetails: (orderId: string) => void, user: User }> = ({ onViewDetails, user }) => {
    ...
    useEffect(() => {
        const fetchAllOrders = async () => {
            try {
                // Assuming there's an API to get all orders, for now we'll use a placeholder
                // const orders = await getAllOrders();
                setAllOrders([]); // Placeholder
            } catch (error) {
...
const UserManagement: React.FC<{ currentUser: User }> = ({ currentUser }) => {
    ...
    const assignableRoles = [Role.CUSTOMER, Role.ORDER_PROCESSOR, Role.ADMIN];
...
const SiteManagement: React.FC = () => {
    ...
    const fetchContent = async () => {
        setIsLoading(true);
        try {
            const [siteData, adsData] = await Promise.all([
                getSiteContent(),
                getAdvertisements()
            ]);
            setContent(siteData);
    ...
    const handleCreateAd = async () => {
        if (!newAdImage || !newAdTitle.trim()) {
            alert('Please provide both title and image for the advertisement.');
            return;
        }

        const formData = new FormData();
        formData.append('title', newAdTitle);
        formData.append('linkUrl', newAdLink);
    ...
    const handleAddHeroMedia = async (file: File, type: 'image' | 'video') => {
        const formData = new FormData();
        formData.append('type', type);
        formData.append('media', file);

        try {
            await createHeroMedia(formData);
            await fetchContent();
    ...
    const handleDeleteHeroMedia = async (id: string) => {
        if (!confirm('Are you sure you want to delete this hero media?')) return;

        try {
            await deleteHeroMedia(id);
            await fetchContent();
            alert('Hero media deleted successfully!');
        } catch (err) {
            alert('Failed to delete hero media.');
        }
    };
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700/50">
                <h3 className="text-xl font-bold mb-4">Hero Section Display Mode</h3>
                <div className="flex gap-4">
                    {['video', 'slideshow', 'image'].map(mode => (
                        <button
                            key={mode}
                            onClick={() => handleUpdate('heroDisplayMode', mode)}
                            className={`px-4 py-2 rounded-md text-sm font-bold capitalize ${
                                content.heroDisplayMode === mode
                ...
                <p className="text-sm text-gray-400 mt-2">
                    Current mode: <span className="font-semibold capitalize">{content.heroDisplayMode}</span>
                </p>
            </div>

            {/* Hero Media Management */}
            <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700/50">
                <h3 className="text-xl font-bold mb-4">Hero Background Media</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                    {content.heroMedia.map(media => (
                        <div key={media.id} className="relative bg-gray-700/50 rounded-lg overflow-hidden">
                            {media.type === 'video' ? (
                                <video src={media.url} className="w-full h-32 object-cover" controls />
                            ) : (
                                <img src={media.url} alt="Hero media" className="w-full h-32 object-cover" />
                            )}
                            <button
                    ...
                    <div>
                        ...
                        <input
                            ...
                            onChange={(e) => {
                                ...
                                if (file) handleAddHeroMedia(file, 'image');
                            }}
                            className="w-full p-2 bg-gray-700/50 rounded border border-gray-600"
                        />
                    </div>
                    <div>
                        <label className="block text-sm text-gray-400 mb-2">Add Video</label>
                        <input
                            type="file"
                            accept="video/*"
                            onChange={(e) => {
                                const file = e.target.files?.[0];
                                if (file) handleAddHeroMedia(file, 'video');
                            }}
                            className="w-full p-2 bg-gray-700/50 rounded border border-gray-600"
                        />
                    </div>
            ...
            <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700/50">
                <h3 className="text-xl font-bold mb-4">Homepage Advertisements</h3>
                <div className="space-y-4 mb-6">
                    {advertisements.map(ad => (
                        <div key={ad.id} className="flex items-center gap-4 p-4 bg-gray-700/30 rounded-lg">
                            <img src={ad.imageUrl} alt={ad.title} className="w-16 h-16 object-cover rounded" />
                            <div className="flex-1">
                                <input
                                    type="text"
                                    defaultValue={ad.title}
                                    onBlur={(e) => handleUpdateAd(ad.id, { title: e.target.value })}
                                    className="w-full p-2 bg-gray-600/50 rounded border border-gray-500 text-white"
                                />
                                <input
                                    type="text"
                                    defaultValue={ad.linkUrl || ''}
                                    onBlur={(e) => handleUpdateAd(ad.id, { linkUrl: e.target.value })}
                                    placeholder="Link URL (optional)"
                                    className="w-full mt-1 p-2 bg-gray-600/50 rounded border border-gray-500 text-white"
                                />
                            </div>
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    className={`px-3 py-1 rounded text-xs font-bold ${
                                        ad.isActive ? 'bg-green-500 hover:bg-green-600' : 'bg-red-500 hover:bg-red-600'
                                    }`}
                                >
                ...
                <div className="border-t border-gray-600 pt-4">
                    <h4 className="text-lg font-semibold mb-4">Add New Advertisement</h4>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="flex gap-2">
                            <input
                                ...
                                className="flex-1 p-2 bg-gray-700/50 rounded border border-gray-600 text-sm"
                            />
                            <button
                                onClick={handleCreateAd}
                                className="px-4 py-2 bg-cyan-500 hover:bg-cyan-600 text-white rounded font-bold"
                            >
                                Add
                            </button>
                        </div>
...
const DashboardPage: React.FC<DashboardPageProps> = ({ user, onLogout }) => {
  ...
  const renderContent = () => {
    switch (activeView) {
      ...
      // User Views
      case 'orders':
        if (user.role === Role.CUSTOMER) return <UserOrders onViewDetails={handleViewOrderDetails} user={user} />;
